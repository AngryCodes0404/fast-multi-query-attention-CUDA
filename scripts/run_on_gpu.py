...
def setup_and_build():
    ...
    try:
        subprocess.run(
            [sys.executable, "setup.py", "build_ext", "--inplace"], check=True
        )
...
def benchmark_real_performance():
    ...
    for batch_size, seq_len, head_dim in configs:
        ...
        mqa_cuda = FastMQAttention(
            num_heads=num_heads, head_dim=head_dim, use_cuda=True
        )
        ...
        # Benchmark PyTorch
        mqa_pytorch = FastMQAttention(
            num_heads=num_heads, head_dim=head_dim, use_cuda=False
        )
