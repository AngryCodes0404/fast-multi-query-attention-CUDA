...
print(
    f"• Reduced memory usage by {best_memory_reduction:.0f}% with Multi-Query Attention"
)
print(
    f"• Optimized throughput to {peak_throughput/1000:.0f}K tokens/second on Tesla T4"
)
print()
print("💼 ALTERNATIVE RESUME BULLETS:")
print(f"• Implemented CUDA kernel processing {peak_throughput:,.0f} tokens/second")
print(
    f"• Reduced transformer KV-cache from {memory_mha:.1f}MB to {memory_mqa:.1f}MB ({best_memory_reduction:.0f}% reduction)"
)
print(
    f"• Optimized memory bandwidth utilization achieving {best_cpu_speedup:.0f}x speedup over CPU"
)
