...
for batch_size, seq_len, head_dim, num_heads in configs:
    ...
    # Throughput
    total_ops = (
        2 * batch_size * num_heads * seq_len * seq_len * head_dim
    )  # Approximate FLOPs
    ...
    results.append(
        {"config": f"B{batch_size}_S{seq_len}_H{num_heads}_D{head_dim}", **metrics}
    )
...
print(
    f"• Reduced memory usage by {np.mean(memory_reductions):.0f}% compared to standard MHA"
)
