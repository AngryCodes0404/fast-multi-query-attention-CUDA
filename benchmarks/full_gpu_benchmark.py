...
print(
    f"Average Memory Reduction: {np.mean([r['memory_reduction'] for r in results]):.1f}%"
)
...
for r in results:
    print(
        f"| B{r['batch_size']}_S{r['seq_len']}_H{r['num_heads']} | {r['cuda_ms']:.2f} | {r['pytorch_ms']:.2f} | {r['speedup']:.2f}x | {r['memory_reduction']:.1f}% |"
    )
